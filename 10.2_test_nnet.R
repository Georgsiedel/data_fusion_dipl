...
### 10. Test und Auswertung NNET-Modell

#Loeschen der ersten Spalte, die automatisch hinzugefügt wird
donor <- donor[, -1]
recipient <- recipient[, -1]

# 10.1 Erstellung Variablen, Funktionen und Kontigenztabellen für Verteilungsauswertung

# 10.1.1 Erstellung Pearson's C Funktion
...
# 10.1.2 leere Vektoren fÃ¼r for Schleife
...
# 10.2 Training und Test des Modells

#for-Schleife für multiple Wiederholungen der Berechnung aller Kennwerte

for (w in 1:1) {
  
  print(paste("Bearbeitung Simulationsdurchlauf", w))
...
# 10.3 Test/Vorhersage mit predict-Funktion
...
#write.csv2(recipient, paste(daten,"/ergebnisse/10_fusionsdatensatz_nnet.csv", sep = ""))

# 10.4 Auswertung Genauigkeit (Ebene 1)

accuracy <- confusionMatrix(prediction, AccidentType)$overall["Accuracy"]
print(accuracy)
print(confusionMatrix(prediction, AccidentType)$byClass)

#10.5 Erstellung Variablen für die Verteilungsauswertung
...
# 10.5.1 Erstellung der decoded-Variablen zur Auswertung der bedingten Verteilungen
...
# 10.5.2 Erstellung Kontigenztabellen für Ebene 4
...
# 10.5.3 gesamter originaler Datensatz
orig_recipient <- select(recipient, -AccidentType_pred)
d.gesamt.data = rbind(orig_recipient, donor)

# 10.6 Auswertung der gemeinsamen Verteilung (Ebene 2)

# 10.6.1 Vergleich der Gesamtverteilung von fXZ, Vorhergesagte Variable im recipient vs. tatsächliche Variable im recipient
...
# 10.6.2 Vergleich der Gesamtverteilung von fXZ, Vorhergesagte Variable im recipient vs. tatsächliche Variable im Gesamtdatensatz
...
# 10.6.3 Vergleich der Verteilung von fz
...
# 10.6.4 Vergleich der tatsächlichen Gesamtverteilungen von donor und recipient als Maßstab der Ungebnauigkeit des Datensplits bzw. Randomness der HD
...
# 10.7 Auswertung der Randverteilung (Ebene 4)

# 10.7.1 Vergleich der Gesamtverteilung von fXZ vom fusionierten Datensatz mit dem Spenderdatensatz --> real testbar
...
#10.7.2 Vergleich der Randverteilung von Z (AccidentType) --> imputierte Variable
...
# 10.7.3 Vergleich der Verteilung von fXZ --> jeweils fÃ¼r jede XZ Kombination 
...
# 10.8 Speichern der multiplen Durchläufe als Ergebnis in Vektoren und Ausgabe

# 10.8.1 Erhalt der einzelnen Werte 

vec.accuracy <- c(vec.accuracy,accuracy)

# 10.8.2 Erhalt der gemeinsamen Verteilung
...
# 10.8.3 Erhalt der Randverteilung
...
# 10.9 Zusammenfassung in Dataframes
...
#saveRDS(df.Ebene1, paste(daten,"/Ergebnisse/10_df_Ebene1_nnet.rds", sep = ""))
#saveRDS(df.Ebene2, paste(daten,"/Ergebnisse/10_df_Ebene2_nnet.rds", sep = ""))
#saveRDS(df.Ebene4, paste(daten,"/Ergebnisse/10_df_Ebene4_nnet.rds", sep = ""))
