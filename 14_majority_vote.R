...
### 14. Test und Auswertung Majority-Vote-Ensembling
...
### 14.6 Ergebniserfassung aller Classifier und Rekombination per Majority Vote zu einer gemeinsamen Abstimmung

#hier: 5 Votings, bei 1vs1vs1vs1vs1 --> RF, bei 2:2:1 --> DHD bzw. nnet
...
#Erstellen neuer Spalte für Majority voting und ausfüllen mit RF-Ergebnis, wenn alle Ergebnisse verschieden sind
ergebnisse$AccidentType_majorityvote <- with(ergebnisse, ifelse(length(levels(as.factor(ergebnisse[,2:6]))) == 5, ergebnisse$AccidentType_rf, "0"))

c <- vector()

for (i in 1:nrow(ergebnisse)) {
  
  c <-as.factor(ergebnisse[i,2:6])
  
  if(length(levels(c)) == 1 | length(levels(c)) == 2 | length(levels(c)) == 4){
    anzahl <- hist(as.numeric(as.character(c)), breaks = c(0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5), plot = FALSE)
    anzahl <- anzahl$counts
    ergebnisse$AccidentType_majorityvote[i] <- which.max(anzahl)
  }
  if(length(levels(c)) == 3){
    anzahl <- hist(as.numeric(as.character(c)), breaks = c(0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5), plot = FALSE)
    anzahl <- anzahl$counts
    if(max(anzahl) == 3){
      ergebnisse$AccidentType_majorityvote[i] <- which.max(anzahl)
    ...
    if(max(anzahl) == 2){
      if(ergebnisse$AccidentType_dhd[i] == ergebnisse$AccidentType_rf[i] | ergebnisse$AccidentType_dhd[i] == ergebnisse$AccidentType_xgb[i] | ergebnisse$AccidentType_dhd[i] == ergebnisse$AccidentType_nnet[i] | ergebnisse$AccidentType_dhd[i] == ergebnisse$AccidentType_svm[i]){
        ergebnisse$AccidentType_majorityvote[i] <- ergebnisse$AccidentType_dhd[i]
      } else {
        ergebnisse$AccidentType_majorityvote[i] <- ergebnisse$AccidentType_nnet[i]
      }                
    }
  }
}

#paralleles Ensemble 2
#ergebnisse$AccidentType_majorityvote <- ergebnisse$AccidentType_svm
#c <- vector()
#for (i in 1:nrow(ergebnisse)) {
 
#  c <-as.factor(ergebnisse[i,2:6])
 
#  if(ergebnisse$AccidentType_xgb[i] == 1 | ergebnisse$AccidentType_xgb[i] == 4){
#    ergebnisse$AccidentType_majorityvote[i] <- ergebnisse$AccidentType_xgb[i]
#  }
#  if(ergebnisse$AccidentType_nnet[i] == 2 | ergebnisse$AccidentType_rf[i] == 2 | ergebnisse$AccidentType_xgb[i] == 2){
#    ergebnisse$AccidentType_majorityvote[i] <- 2     
#  }
#  if(ergebnisse$AccidentType_nnet[i] == 5 | ergebnisse$AccidentType_rf[i] == 5 | ergebnisse$AccidentType_xgb[i] == 5){
#    ergebnisse$AccidentType_majorityvote[i] <- 5     
#  }
#}

#paralleles Ensemble 4 
#ergebnisse$AccidentType_majorityvote <- ergebnisse$AccidentType_rf
#c <- vector()
#for (i in 1:nrow(ergebnisse)) {
  
# c <-as.factor(ergebnisse[i,2:6])
  
#  if(ergebnisse$AccidentType_rf[i] == 7){
#    ergebnisse$AccidentType_majorityvote[i] <- ergebnisse$AccidentType_dhd[i]
#  }
#}

#paralleles Ensemble 3
#ergebnisse$AccidentType_majorityvote <- ergebnisse$AccidentType_svm
#c <- vector()
#for (i in 1:nrow(ergebnisse)) {
...
#  if(ergebnisse$AccidentType_svm[i] == 6 | ergebnisse$AccidentType_svm[i] == 7){
#    if(ergebnisse$AccidentType_dhd[i] == 2 | ergebnisse$AccidentType_dhd[i] == 5){
#      ergebnisse$AccidentType_majorityvote[i] <- ergebnisse$AccidentType_dhd[i]
#  }
#}
#}
...
for (i in 1:nrow(ergebnisse)) {
  ...
  if (ergebnisse$AccidentType[i] == ergebnisse$AccidentType_majorityvote[i])
    anzGleich = anzGleich + 1
}
print(anzGleich)
...
recipient <- mutate(recipient, AccidentType_pred = ergebnisse$AccidentType_majorityvote)

# 14.7.2 Erstellung Variablen für die Verteilungsauswertung
...
# 14.7.3 Erstellung der decoded-Variablen zur Auswertung der bedingten Verteilungen
...
# 14.7.4 Erstellung Kontigenztabellen für Ebene 4
...
# 14.8.1 Auswertung der gemeinsamen Verteilung (Ebene 2)
...
# 14.8.2 Auswertung der Randverteilung (Ebene 4)
...
# 14.9 Speichern der multiplen Durchläufe als Ergebnis in Vektoren und Ausgabe
...
# 14.10 Zusammenfassung in Dataframes
