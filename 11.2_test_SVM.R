...
### 11. Test und Auswertung SVM-Modell

#Loeschen der ersten Spalte, die automatisch hinzugefügt wird
donor <- donor[, -1]
recipient <- recipient[, -1]

# 11.1 Erstellung Variablen, Funktionen und Kontigenztabellen für Verteilungsauswertung

# 11.1.1 Erstellung Pearson's C Funktion
...
# 11.1.2 leere Vektoren fÃ¼r for Schleife
...
# 11.2 Training und Test des Modells
...
# 11.3 Test/Vorhersage mit predict-Funktion
...
#write.csv2(recipient, paste(daten,"/ergebnisse/11_fusionsdatensatz_svm.csv", sep = ""))

# 11.4 Auswertung Genauigkeit (Ebene 1)

accuracy <- confusionMatrix(prediction, AccidentType)$overall["Accuracy"]
print(accuracy)
print(confusionMatrix(prediction, AccidentType)$byClass)

#11.5 Erstellung Variablen für die Verteilungsauswertung
...
# 11.5.1 Erstellung der decoded-Variablen zur Auswertung der bedingten Verteilungen
...
# 11.5.2 Erstellung Kontigenztabellen für Ebene 4
...
# 11.5.3 gesamter originaler Datensatz
orig_recipient <- select(recipient, -AccidentType_pred)
d.gesamt.data = rbind(orig_recipient, donor)

# 11.6 Auswertung der gemeinsamen Verteilung (Ebene 2)

# 11.6.1 Vergleich der Gesamtverteilung von fXZ, Vorhergesagte Variable im recipient vs. tatsächliche Variable im recipient
...
# 11.6.2 Vergleich der Gesamtverteilung von fXZ, Vorhergesagte Variable im recipient vs. tatsächliche Variable im Gesamtdatensatz
...
# 11.6.3 Vergleich der Verteilung von fz
...
# 11.6.4 Vergleich der tatsächlichen Gesamtverteilungen von donor und recipient als Maßstab der Ungebnauigkeit des Datensplits bzw. Randomness der HD
...
# 11.7 Auswertung der Randverteilung (Ebene 4)

# 11.7.1 Vergleich der Gesamtverteilung von fXZ vom fusionierten Datensatz mit dem Spenderdatensatz --> real testbar
...
#11.7.2 Vergleich der Randverteilung von Z (AccidentType) --> imputierte Variable
...
# 11.7.3 Vergleich der Verteilung von fXZ --> jeweils fÃ¼r jede XZ Kombination 
...
# 11.8 Speichern der multiplen Durchläufe als Ergebnis in Vektoren und Ausgabe

# 11.8.1 Erhalt der einzelnen Werte 

vec.accuracy <- c(vec.accuracy,accuracy)

# 11.8.2 Erhalt der gemeinsamen Verteilung
...
# 11.8.3 Erhalt der Randverteilung
...
# 11.9 Zusammenfassung in Dataframes
...
#saveRDS(df.Ebene1, paste(daten,"/Ergebnisse/11_df_Ebene1_svm.rds", sep = ""))
#saveRDS(df.Ebene2, paste(daten,"/Ergebnisse/11_df_Ebene2_svm.rds", sep = ""))
#saveRDS(df.Ebene4, paste(daten,"/Ergebnisse/11_df_Ebene4_svm.rds", sep = ""))
